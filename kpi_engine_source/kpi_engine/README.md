...
A prototype built for BusinessIntelligence.ai Round 2: detects material KPI movements,
ranks their drivers using the appropriate analytical method for each (not just an LLM
...
needed). We're running on `gemini-flash-lite-latest`, confirmed working as of this
writing — but Gemini's free-tier model names shift over time, so if you get a 404,
check the current list with `curl -s "https://generativelanguage.googleapis.com/v1beta/models?key=$GEMINI_API_KEY"`
and update `MODEL_NAME` in `llm/gemini_provider.py` to match. Without any key set at
all, swap `GeminiProvider` for `MockLLMProvider` in `run_scenarios.py` and the whole
pipeline still runs end-to-end on deterministic template narratives, no internet
required.
...
- A second LLM provider (e.g. Anthropic Claude) as an additional fallback tier — this
  is actually already written (`llm/anthropic_provider.py`), just not used in this
  submission since a paid/trial-credited account wasn't available in time. The
  interface (`llm/base_provider.py`) made adding it a ~30 line change either way.
