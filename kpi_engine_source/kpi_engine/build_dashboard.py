...
function mdToHtml(text){
  // Lightweight markdown handling for LLM output: escape first (safety),
  // then render **bold** as actual bold. Newlines already render via
  // white-space:pre-wrap on .narrative, so they're left alone here.
  return escapeHtml(text).replace(/\*\*(.+?)\*\*/g, '<strong>$1</strong>');
}
function narrativeBlock(result){
  return `<div class="narrative"><span class="tag badge b-llm">LLM narrative — ${result.narrative_model}</span>${mdToHtml(result.narrative)}</div>`;
}
