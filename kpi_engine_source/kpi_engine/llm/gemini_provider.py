...
MODEL_NAME = "gemini-flash-lite-latest"   # confirmed working via real run - Aug 2026
        ...
        for attempt in range(MAX_RETRIES):
            try:
                resp = requests.post(
                    ...
                    timeout=60,   # free-tier latency can be high under load
                )
            except requests.exceptions.RequestException as e:
                # Covers read timeouts, connection resets, DNS blips - all
                # transient network conditions, not something a retry can't fix.
                last_error = str(e)
                if attempt < MAX_RETRIES - 1:
                    time.sleep(RETRY_BACKOFF_SECONDS * (attempt + 1))
                    continue
                raise RuntimeError(
                    f"Gemini request failed after {MAX_RETRIES} attempts (network/timeout): {last_error}"
                ) from e
